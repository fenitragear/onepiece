    ...
    var Editor = function () {
        ...
        var triggerOnChange = this.triggerOnChange = function ($editable) {
            ...
            if (onChange && cpt === 1) {
                ...
                if (!$editable.html().match('<p>')) {
                    context.formatPara($editable);
                }
            }
            cpt--;
        };
        ...
        var commands = ['bold', 'italic', 'underline', 'strikethrough', 'superscript', 'subscript',
            ...
            'backColor', 'insertHorizontalRule', 'fontName'];
        ...
        this.foreColor = function ($editable, value) {
            document.execCommand('foreColor', false, value);
            document.execCommand('fontSize', false, 3);

            $editable.find('font[size=3]').find('a').css('color', value);
        ...
        this.fontSize = function ($editable, value) {
            document.execCommand('fontSize', false, 3);
            $editable.find('font[size=3]').find('font').removeAttr('style');
            ...
            $editable.find('a').filter(function () {
                return this.style.fontSize === 'medium';
            }).css('font-size', value + 'px');
        ...
        this.color = function ($editable, sObjColor) {
            ...
            var backColor = oColor.backColor;
    ...
    var Dialog = function () {
        ...
        this.showLinkDialog = function ($editable, $dialog, linkInfo) {
            return $.Deferred(function (deferred) {
                ...
                var $linkBtn = $linkDialog.find('.note-link-btn');
                var $linkFormCheckbox = $linkDialog.find('.note-link-form-checkbox');
                var $linkFormCheckboxLabel = $linkDialog.find('.note-link-form-checkbox-label').show();
                var $openInNewWindow = $linkDialog.find('input[type=checkbox].blank');

                //wylog start
                var url =  window.location.href;
                var urlArray = url.split("/");
                var clientId = urlArray[urlArray.length - 1];

                if (clientId === '') {
                    clientId = urlArray[urlArray.length - 2];
                }

                $.post('../server/service/baseUrl.php', {session: clientId}, function (data) {
                    $linkDialog.one('shown.bs.modal', function () {
                        $linkDialog.find('.note-link-btn-cancel').on('click', function () {
                        ...
                        if (data.formShow) {
                            if (linkInfo.url.indexOf(linkForm) >= 0) {
                                $($linkUrl).attr('readonly', true);
                                $($linkFormCheckbox).attr('checked',"checked");
                            } else {
                                $($linkUrl).attr('readonly', false);
                                $($linkFormCheckbox).attr('checked', false);
                            }

                            $($linkFormCheckboxLabel).show();
                        } else {
                            $($linkFormCheckboxLabel).hide();
                        }

                        toggleBtn($linkBtn, $linkUrl);

                        $($linkFormCheckbox).change(function () {
                            if ($(this).is(":checked")) {
                                $($linkUrl).attr('readonly', true);
                                $linkUrl.val(linkForm);
                                toggleBtn($linkBtn, linkForm);
                            } else {
                                $linkUrl.val("");
                                $($linkUrl).attr('readonly', false);
                                toggleBtn($linkBtn, "");
                        ...
                        $linkText.keyup(function () {
                            ...
                            linkInfo.text = linkForm + $linkText.val();
                        });
        ...
        var tplButtonInfo = {
            ...
            fontname: function (lang, options) {
                var items = options.fontNames.reduce(function (memo, v) {
                    ...
                    "<i class=\"fa fa-check icon-ok\"></i>  <span style=\"font-family: '" + v + "'\">" + v + '</span>' +
                    '</a></li>';
                }, '');
                ...
                return tplButton(label, {
                    ...
                    dropdown: '<ul class="dropdown-menu font-names-menu">' + items + '</ul>'
                });
            },
            ...
            var tplLinkDialog = function () {
                var body = '<div class="form-group">' +
                    ...
                    (!options.disableLinkTarget ?
                        '<div class="checkbox">' +
                        '<label>' + '<input type="checkbox" class="blank" checked> ' +
                    ...
                    '<div class="checkbox">' +
                    '<label class="note-link-form-checkbox-label">' + '<input type="checkbox"  class="note-link-form-checkbox"> Lien vers le formulaire</label>' +
                    '</div>'
                ;

                var footer = '<button href="#" class="btn btn-primary note-link-btn disabled" disabled>Valider</button>';
                footer += '<button href="#" type="button" class="btn btn-warning note-link-btn-cancel" >Annuler</button>';
        ...
        this.createLayoutByFrame = function ($holder, options) {
            ...
            for (var idx = 0, len = options.toolbar.length; idx < len; idx++) {
                ...
                for (var i = 0, btnLength = groupButtons.length; i < btnLength; i++) {
                    ...
                    if (!$.isFunction(tplButtonInfo[groupButtons[i]])) {
                        continue;
                    }
